...
describe('WordPressApiClient', () => {
	...
	describe('getBlockTypes', () => {
		...
		it('returns block types with attributes', async () => {
			...
			expect(imageBlock.attributes.caption).toEqual({
				type: 'rich-text',
				source: 'rich-text',
			});
	...
	describe('note methods', () => {
		...
		describe('checkNotesSupport', () => {
			it('returns true when the endpoint succeeds', async () => {
				...
				const url = fetchMock.mock.calls[0][0] as string;
				expect(url).toBe(
					'https://example.com/wp-json/wp/v2/comments?type=note&per_page=1'
				);
			});
			...
			it('re-throws non-API errors (e.g. network failure)', async () => {
				...
				await expect(client.checkNotesSupport()).rejects.toThrow(
					'fetch failed'
				);
			});
		...
		describe('updateNote', () => {
			it('posts to /wp/v2/comments/{noteId} with content', async () => {
				...
				const client = createClient();
				const result = await client.updateNote(10, {
					content: 'Updated',
				});
				...
				expect(url).toBe(
					'https://example.com/wp-json/wp/v2/comments/10'
				);
				expect(options.method).toBe('POST');
				const body = JSON.parse(options.body as string) as {
					content: string;
		...
		describe('deleteNote', () => {
			it('sends DELETE to /wp/v2/comments/{noteId} with force=true', async () => {
				...
				expect(url).toBe(
					'https://example.com/wp-json/wp/v2/comments/10?force=true'
				);
	...
	describe('updatePost', () => {
		it('posts to /wp/v2/posts/{id}?context=edit with JSON body', async () => {
			const updatedPost: WPPost = {
				...fakePost,
				title: { rendered: 'Updated', raw: 'Updated' },
			};
		...
		it('throws on error', async () => {
			...
			const client = createClient();
			await expect(
				client.updatePost(999, { title: 'Nope' })
			).rejects.toThrow(WordPressApiError);
		});
	});
	...
	describe('createTerm', () => {
		...
		it('throws on duplicate term', async () => {
			...
			const client = createClient();
			await expect(
				client.createTerm('categories', 'Existing')
			).rejects.toThrow(WordPressApiError);
		});
	});
