...
describe('note tools', () => {
			...
			const fakeNotes = [
				{
					...
					date: '2026-03-20',
					content: {
						rendered: '<p>Needs more detail</p>',
						raw: 'Needs more detail',
					},
					status: 'hold',
					type: 'note',
				},
	...
	describe('wp_add_note', () => {
		...
		it('returns error on failure', async () => {
			(
				session.addNote as ReturnType<typeof import('vitest').vi.fn>
	...
	describe('wp_reply_to_note', () => {
		...
		it('returns error on failure', async () => {
			(
				session.replyToNote as ReturnType<typeof import('vitest').vi.fn>
	...
	describe('wp_resolve_note', () => {
		...
		it('returns error on failure', async () => {
			(
				session.resolveNote as ReturnType<typeof import('vitest').vi.fn>
			).mockRejectedValue(new Error('Note not found'));
	...
	describe('wp_update_note', () => {
		it('calls session.updateNote and returns confirmation', async () => {
			...
			const result = await tool.handler({
				noteId: 1,
				content: 'Revised feedback',
			});

			expect(session.updateNote).toHaveBeenCalledWith(
				1,
				'Revised feedback'
			);
		...
		it('returns error on failure', async () => {
			(
				session.updateNote as ReturnType<typeof import('vitest').vi.fn>
			).mockRejectedValue(new Error('Note not found'));
