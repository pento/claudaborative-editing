...
describe('metadata tools', () => {
	...
	beforeEach(() => {
		...
		registerToolDefinitions(server, session, metadataTools);
	});
	...
	describe('wp_list_categories', () => {
		it('lists categories with counts', async () => {
			...
			expect(
				session.listCategories as ReturnType<typeof vi.fn>
	...
	describe('wp_list_tags', () => {
		...
		it('returns error on failure', async () => {
			(session.listTags as ReturnType<typeof vi.fn>).mockRejectedValue(
				new Error('API error')
			);
			const tool = server.registeredTools.get('wp_list_tags');
			assertDefined(tool);
	...
	describe('wp_set_status', () => {
		...
		it('shows old status from current post', async () => {
			...
			expect(result.content[0].text).toBe(
				'Status changed from "publish" to "publish".'
			);
		});

		it('returns error on failure', async () => {
			(
				session.setPostStatus as ReturnType<typeof vi.fn>
	...
	describe('wp_set_categories', () => {
		it('sets existing categories', async () => {
			(
				session.setCategories as ReturnType<typeof vi.fn>
			).mockResolvedValue({
				post: fakePost,
		...
		it('handles mix of existing and created categories', async () => {
			...
			expect(result.content[0].text).toBe(
				'Categories set: Tech, AI (created).'
			);
		});
		...
		it('returns error on failure', async () => {
			(
				session.setCategories as ReturnType<typeof vi.fn>
			).mockRejectedValue(new Error('Not editing'));
	...
	describe('wp_set_tags', () => {
		it('sets tags and returns confirmation', async () => {
			...
			expect(result.content[0].text).toBe(
				'Tags set: tutorial, beginner (created).'
			);
		});
	...
	describe('wp_set_featured_image', () => {
		it('sets featured image and returns confirmation', async () => {
			...
			expect(result.content[0].text).toBe(
				'Featured image set to attachment 42.'
			);
		});
		...
		it('returns error on failure', async () => {
			(
				session.setFeaturedImage as ReturnType<typeof vi.fn>
			).mockRejectedValue(new Error('Invalid attachment ID'));
	...
	describe('wp_set_date', () => {
		it('sets date and returns confirmation with the date', async () => {
			...
			expect(result.content[0].text).toBe(
				'Publication date set to 2026-04-01T09:00:00.'
			);
		});
		...
		it('resets date when passed empty string', async () => {
			...
			expect(result.content[0].text).toBe(
				'Publication date reset to default.'
			);
		});
	...
	describe('wp_set_comment_status', () => {
		...
		it('returns error on failure', async () => {
			(
				session.setCommentStatus as ReturnType<typeof vi.fn>
