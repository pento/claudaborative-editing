...
describe('post tools', () => {
	...
	describe('wp_list_posts', () => {
		...
		it('shows message when no posts found', async () => {
			(
				session.listPosts as ReturnType<typeof import('vitest').vi.fn>
			).mockResolvedValue([]);
	...
	describe('wp_close_post', () => {
		...
		it('returns error on failure', async () => {
			(
				session.closePost as ReturnType<typeof import('vitest').vi.fn>
			).mockImplementation(() => {
				throw new Error(
	...
	describe('wp_create_post', () => {
		it('creates post and returns rendered content', async () => {
			...
			expect(session.createPost).toHaveBeenCalledWith({
				title: 'New Post',
				content: undefined,
			});
			expect(result.content[0].text).toContain(
				'Created and opened post 42'
			);
		});

		it('returns error on failure', async () => {
			(
				session.createPost as ReturnType<typeof import('vitest').vi.fn>
