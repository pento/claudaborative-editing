...
describe('read tools', () => {
  ...
  beforeEach(() => {
    ...
    session = createMockSession({
      ...
      user: fakeUser,
      post: fakePost,
      postContent: 'Title: "My Great Post"\n\n[0] core/paragraph\n  "Hello world"',
  ...
  describe('wp_read_post', () => {
    ...
    it('returns error when not editing', async () => {
      (session.readPost as ReturnType<typeof import('vitest').vi.fn>).mockImplementation(() => {
  ...
  describe('wp_read_block', () => {
    ...
    it('supports dot notation for nested blocks', async () => {
      (session.readBlock as ReturnType<typeof import('vitest').vi.fn>).mockReturnValue(
        '[2.1] core/list-item\n  "Nested item"',
      );
    ...
    it('returns error for invalid index', async () => {
      (session.readBlock as ReturnType<typeof import('vitest').vi.fn>).mockImplementation(() => {
