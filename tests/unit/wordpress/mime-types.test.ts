...
import {
	getMimeType,
	getMediaCategory,
} from '../../../src/wordpress/mime-types.js';
...
describe('getMimeType', () => {
	...
	it('throws for unknown extensions', () => {
		expect(() => getMimeType('file.xyz')).toThrow(
	...
	it('throws for files with no extension', () => {
		expect(() => getMimeType('Makefile')).toThrow(
...
describe('getMediaCategory', () => {
	...
	it('returns "application" for document/other MIME types', () => {
		...
		expect(getMediaCategory('application/octet-stream')).toBe(
			'application'
		);
	});
});
