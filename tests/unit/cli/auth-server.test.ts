...
function simulateRequest(url: string): {
	...
	if (!capturedHandler)
		throw new Error('No handler captured — startAuthFlow not called?');

	const req = {
		url,
		headers: { host: `127.0.0.1:${MOCK_PORT}` },
...
describe('startAuthFlow', () => {
	it('resolves the handle once the server is listening', async () => {
		...
		const handle = await startAuthFlow('https://example.com', {
			openBrowser,
		});
	...
	it('uses the site URL as the base of the auth URL', async () => {
		...
		const handle = await startAuthFlow('https://custom.wp-site.org', {
			openBrowser,
		});

		expect(handle.authUrl).toMatch(
			/^https:\/\/custom\.wp-site\.org\/wp-admin\//
		);
	...
	it('uses the caller-provided siteUrl, not the WordPress callback site_url', async () => {
		...
		const handle = await startAuthFlow('https://my-normalised-url.com', {
			openBrowser,
		});
		...
		expect(result.credentials.siteUrl).toBe(
			'https://my-normalised-url.com'
		);
	});
	...
	it('serves an HTML rejection page on denial', async () => {
		...
		const response = simulateRequest(
			`/callback?state=${MOCK_STATE}&rejected=true`
		);
		expect(response.status).toBe(200);
		expect(response.headers['Content-Type']).toContain('text/html');
	...
	it('returns 403 when callback has invalid state', async () => {
		...
		const response = simulateRequest(
			'/callback?state=wrong&user_login=admin&password=xxxx'
		);
	...
	it('rejects the handle when server emits an error', async () => {
		...
		await expect(
			startAuthFlow('https://example.com', { openBrowser })
	...
	it('does not resolve result twice if callback arrives after abort', async () => {
		...
		simulateRequest(
			`/callback?state=${MOCK_STATE}&user_login=other&password=second`
		);
...
describe('openBrowserDefault', () => {
	...
	it('resolves even when execFile fails', async () => {
		...
		// Should not reject
		await expect(
			openBrowserDefault('https://example.com')
