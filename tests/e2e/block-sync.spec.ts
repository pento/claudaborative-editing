    ...
    .poll(async () => {
      return page.evaluate(() => {
        const wpGlobal = globalThis as typeof globalThis & {
          wp?: {
            data?: {
              select: (store: string) => { getBlocks: () => Array<unknown> };
            };
          };
        };
...
test.describe('block sync', () => {
  test('MCP inserts blocks visible in browser', async ({ page }) => {
    ...
    try {
      ...
      await expect
        .poll(() => getBrowserBlocks(page), { timeout: 30_000, intervals: [1000] })
        .toEqual(expect.arrayContaining([expect.objectContaining({ name: 'core/heading' })]));
        ...
        .toEqual(
          expect.arrayContaining([
            expect.objectContaining({
              name: 'core/heading',
              attributes: expect.objectContaining({ content: 'Original heading' }),
            }),
            expect.objectContaining({
              name: 'core/heading',
              attributes: expect.objectContaining({ content: 'Inserted heading', level: 3 }),
            }),
            expect.objectContaining({
              name: 'core/paragraph',
              attributes: expect.objectContaining({ content: 'Inserted paragraph' }),
            }),
          ]),
        );
    ...
    try {
      ...
      await expect
        ...
        .toEqual(
          expect.arrayContaining([
            expect.objectContaining({
              attributes: expect.objectContaining({ content: 'First paragraph' }),
            }),
            expect.objectContaining({
              attributes: expect.objectContaining({ content: 'Second paragraph' }),
            }),
          ]),
        );
  ...
  test('browser edits visible in MCP', async ({ page }) => {
    ...
    try {
      ...
      await expect
        .poll(
          async () => {
            const post = await callToolOrThrow(client, 'wp_read_post');
    ...
    try {
      ...
      await expect
        .poll(
          async () => {
            const blocks = await getBrowserBlocks(page);
            return { content: blocks[0]?.attributes?.content, level: blocks[0]?.attributes?.level };
          },
          { timeout: 30_000, intervals: [1000] },
        )
