...
const PARAGRAPH_CONTENT = '<!-- wp:paragraph --><p>Test paragraph</p><!-- /wp:paragraph -->';
...
test.describe('deleted post detection', () => {
  test('detects permanently deleted post and errors on editing operations', async () => {
    ...
    const postId = await createDraftPost('E2E deleted-post permanent', PARAGRAPH_CONTENT);
    ...
    try {
      ...
      await expect
        .poll(
          async () => {
            const result = await callTool(client, 'wp_update_block', {
              index: '0',
        ...
        .toMatchObject({ isError: true, text: expect.stringContaining('deleted') });
  ...
  test('detects trashed post and errors on editing operations', async () => {
    ...
    const postId = await createDraftPost('E2E deleted-post trash', PARAGRAPH_CONTENT);
    const appPassword = getSharedAppPassword();
    const { client, close, stderr } = await createMcpTestClient();
    ...
    try {
      ...
      await expect
        ...
        .toMatchObject({ isError: true, text: expect.stringContaining('trash') });
