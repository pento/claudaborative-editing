...
The repo pins `"packageManager": "npm@12.0.2"` (corepack requires an exact version) and expresses the real floor as a range in `devEngines.packageManager` (`>=12.0.0`). `devEngines` is dev-only — unlike `engines`, it is not applied to people installing the published package. npm 11 and npm 12 build different trees from the same lockfile, so the floor matters.

**`onFail` must stay `warn`, not `error`.** `actions/setup-node` enables npm caching automatically when `devEngines.packageManager` names npm, and implements it by running `npm config get cache` in the workspace — with the runner's _bundled_ npm, before any `npm install -g npm@...` step can run. With `onFail: "error"` that command exits `EBADDEVENGINES` and the whole `setup-node` step fails, taking every npm job with it. This is not specific to a `setup-node` major: any caching setup has to probe npm before the pinned version is installed. A contributor on the wrong npm still gets a loud `npm warn EBADDEVENGINES`, and a lockfile generated by the wrong npm is caught by `npm ci` in CI anyway.
