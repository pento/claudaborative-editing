...
function formatTermList(terms: Array<{ name: string; id: number; count?: number; parent?: number }>): string {
...
export function registerMetadataTools(server: McpServer, session: SessionManager): void {
  server.tool(
    ...
    async ({ search }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `Categories:\n${formatTermList(terms)}` }] };
      } catch (err) {
  ...
  server.tool(
    ...
    async ({ search }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `Failed to list tags: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    async ({ status }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `Status changed from "${oldStatus}" to "${status}".` }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set status: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    {
      categories: z.array(z.string()).min(1).describe('Category names to assign (e.g., ["Tech", "News"])'),
    },
    async ({ categories }) => {
      try {
        const { resolved } = await session.setCategories(categories);
        const parts = resolved.map((r) => r.created ? `${r.name} (created)` : r.name);
        return { content: [{ type: 'text' as const, text: `Categories set: ${parts.join(', ')}.` }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set categories: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    async ({ tags }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `Failed to set tags: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    async ({ excerpt }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `Failed to set excerpt: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    {
      attachmentId: z.number().describe('Media attachment ID (from wp_upload_media), or 0 to remove'),
    },
    async ({ attachmentId }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: `Featured image set to attachment ${attachmentId}.` }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set featured image: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    {
      date: z.string().describe('Publication date in ISO 8601 format (e.g., "2026-04-01T09:00:00"), or empty string to clear'),
    },
    async ({ date }) => {
      try {
        const updated = await session.setDate(date);
        if (!date) {
          return { content: [{ type: 'text' as const, text: 'Publication date reset to default.' }] };
        }
        return { content: [{ type: 'text' as const, text: `Publication date set to ${updated.date}.` }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set date: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    async ({ slug }) => {
      try {
        ...
        if (actualSlug !== slug) {
          return { content: [{ type: 'text' as const, text: `Slug set to "${actualSlug}" (adjusted from "${slug}" for uniqueness).` }] };
        }
        return { content: [{ type: 'text' as const, text: `Slug set to "${actualSlug}".` }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set slug: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    'wp_set_sticky',
    'Pin or unpin the post on the front page.',
    {
    ...
    async ({ sticky }) => {
      try {
        ...
        return { content: [{ type: 'text' as const, text: sticky ? 'Post pinned to front page.' : 'Post unpinned.' }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set sticky: ${(err as Error).message}` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    {
      status: z.enum(['open', 'closed']).describe('"open" to enable comments, "closed" to disable'),
    },
    async ({ status }) => {
      try {
        await session.setCommentStatus(status);
        return { content: [{ type: 'text' as const, text: status === 'open' ? 'Comments enabled.' : 'Comments disabled.' }] };
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Failed to set comment status: ${(err as Error).message}` }], isError: true };
      }
    },
  );
}
