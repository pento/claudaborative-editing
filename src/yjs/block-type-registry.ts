...
export class BlockTypeRegistry {
	...
	static fromApiResponse(blockTypes: WPBlockType[]): BlockTypeRegistry {
		...
		for (const blockType of blockTypes) {
			...
			const ancestor = blockType.ancestor?.length
				? blockType.ancestor
				: null;
			const allowedBlocks = blockType.allowed_blocks?.length
				? blockType.allowed_blocks
				: null;

			const supportsInnerBlocks =
				blockType.supports?.allowedBlocks === true;
	...
	static createFallback(): BlockTypeRegistry {
		...
		for (const name of allNames) {
			entries.set(name, {
				...
				richTextAttributes:
					FALLBACK_RICH_TEXT_ATTRIBUTES[name] ?? new Set(),
	...
	isRichTextAttribute(blockName: string, attrName: string): boolean {
		return (
			this.entries.get(blockName)?.richTextAttributes.has(attrName) ??
			false
		);
	}
