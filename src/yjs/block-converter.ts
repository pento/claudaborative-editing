...
export function blockToYMap(
	...
	for (const [key, value] of Object.entries(block.attributes)) {
		if (
			registry.isRichTextAttribute(block.name, key) &&
			typeof value === 'string'
		) {
	...
	if (block.innerBlocks.length > 0) {
		const innerMaps = block.innerBlocks.map((inner) =>
			blockToYMap(inner, registry)
		);
 ...
 */
export function findHtmlSafeChunkEnd(
	text: string,
	offset: number,
