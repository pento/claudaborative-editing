		...
		function render(initial: boolean): void {
			...
			for (let i = 0; i < items.length; i++) {
				...
				const hint = items[i].hint
					? ` \x1B[2m${items[i].hint}\x1B[0m`
					: '';
				process.stdout.write(
					`\x1B[2K  ${pointer} [${check}] ${items[i].label}${hint}\n`
				);
			}
			...
			const donePointer =
				cursor === items.length ? '\x1B[36m❯\x1B[0m' : ' ';
			const doneStyle = doneDisabled ? '\x1B[2m' : '\x1B[1m';
			const doneSuffix = doneDisabled
				? ' (select at least one)'
				: ` (${count} selected)`;
			process.stdout.write(
				`\x1B[2K  ${donePointer} ${doneStyle}Done\x1B[0m${doneSuffix}\n`
			);
		}
